...
public class ImageUtility{
	 ...
	 * @throws IOException
	 */
	public static void writeRects(final byte[][] gray, final ArrayList<ArrayList<Rect>> cols, final String outputFilePath) throws IOException {
		int h = gray.length;
		int w = gray[0].length;
		...
		for (ArrayList<Rect> rects : cols) {
			...
			int newRGB = rgb(random.nextInt(256), random.nextInt(256), random.nextInt(256));
			for (Rect rect : rects) {
				for (int y = rect.y; y < rect.y + rect.h; y++) {
					for (int x = rect.x; x < rect.x + rect.w; x++) {
						if ((y == rect.y || y == rect.y + rect.h - 1) || (x == rect.x || x == rect.x + rect.w - 1)) {
							mutableRGB[y][x] = newRGB;
						}
					}
				}
			}
